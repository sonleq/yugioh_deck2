...
<?php
/* prevent card of same name being enter
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get the form data
    $card_name = $_POST['card_name'];
    $card_type = $_POST['card_type'];
    ...
    if (file_exists($file) && is_readable($file)) {
        ...
        // Check if the card name already exists in the file
        if (strpos($card_data, "Card Name: $card_name") !== false) {
            // If the card name is found, display an error message
            echo "<h2>Error: Card with the name '$card_name' already exists!</h2>";
    ...
    // Generate a unique ID for the card
    $card_id = uniqid();  // Using a unique ID (timestamp-based)

    // Prepare the data to be saved in the text file
    $card_data = "ID: $card_id\n";  // Save the unique ID
    $card_data .= "Card Name: $card_name\n";
    ...
    // Write the card data to the text file
    file_put_contents($file, $card_data, FILE_APPEND);
